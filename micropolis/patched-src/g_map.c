...
MemDrawMap(SimView *view)
{
  ...
  if (!view->x->color) {
    /* A/UX 1-bit: without shared memory the map's XImage header was never
       created (only the dither buffers view->data / view->data8 exist), so
       XPutImage was handed a NULL image and crashed XmacII.  Build a depth-1
       XYBitmap image around the existing dithered buffer, once. */
    if (view->image == NULL && view->data != NULL &&
	view->m_width > 0 && view->m_height > 0) {
      view->image = XCreateImage(view->x->dpy, view->x->visual, 1, XYBitmap, 0,
				 (char *)view->data, view->m_width, view->m_height,
				 8, view->line_bytes);
    }
    if (view->image != NULL && view->data != NULL && view->data8 != NULL) {
      ditherMap(view);
