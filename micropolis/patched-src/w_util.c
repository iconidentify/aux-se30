...
setSpeed(short speed)
{
  ...
  /* A/UX: pace the simulation by wall-clock time.  The original relied on the
     CPU being the bottleneck (sim_delay was ~50us, i.e. "as fast as possible"),
     so on a fast host (emulator / modern CPU) the game ran absurdly fast.  Set
     a real per-speed timer period instead.  The Tk micro-timer delay is a
