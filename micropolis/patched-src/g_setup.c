...
GetObjectXpms(XDisplay *xd, int id, int frames)
{
  ...
  for (i = 0; i < frames; i++) {
    ...
    if (XpmReadFileToPixmap(xd->dpy,
			    ...
			    &attributes) < 0) {
      /* A/UX: the DUX demo ships an incomplete sprite set (e.g. obj1
         lacks frame 4, obj8 is absent entirely).  Missing frames are
         non-fatal: reuse frame 0 if we have it, otherwise None, and
         keep going so the game can still start. */
      fprintf(stderr,
	      "Micropolis: missing sprite \"%s\" (skipping).\n",
	      name);
      if (i > 0) {
	pixmaps[i + i]     = pixmaps[0];
	pixmaps[i + i + 1] = pixmaps[1];
      } else {
	pixmaps[i + i]     = None;
	pixmaps[i + i + 1] = None;
      }
...
GetViewTiles(SimView *view)
{
  ...
  if (view->class == Editor_Class) {
    ...
    sprintf(name, "%s/images/%s", HomeDir,
	    view->x->color ? "tilesbw.xpm" : "tilesbw.xpm");

    switch (view->type) {

    case X_Mem_View:
      if (view->x->big_tile_image == NULL) {
	int xpmrc = XpmReadFileToImage(view->x->dpy, name,
			       &view->x->big_tile_image, NULL,
			       &attributes);
	if (xpmrc < 0) {
	  fprintf(stderr,
		  "Uh oh, Micropolis couldn't read the pixmap file \"%s\" (XpmError %d).\n",
		  name, xpmrc);
