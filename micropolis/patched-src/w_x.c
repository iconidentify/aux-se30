...
CatchXError(Display *dpy, XErrorEvent *err)
{
  GotXError = 1;
  fprintf(stderr,
    "auxsim X error: code=%d request=%d minor=%d resourceid=0x%lx serial=%lu\n",
...
#define GETCOLOR(i, name) \
	...
	if (!pixmaps) {
      ...
      /* A/UX: XmacII crashes drawing depth-1 (XYBitmap) shared-memory pixmaps,
         so fall back to the plain X protocol on a 1-bit (SE/30) display. */
      if (xd->depth == 1) {
	xd->shared = 0;
	fprintf(stderr,
		"Micropolis: 1-bit display - disabling shared memory (XmacII safety).\n");
      }
    }
#else
    xd->shared = 0;
