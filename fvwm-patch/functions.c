		     ...
		     int val1_unit, int val2_unit, MenuRoot *menu, int Module)
{
  ...
  switch (func)
    ...
    case F_ARRANGE_ICONS:
      {
	/* Re-snap every icon into the IconBox grid: forget manual
	 * drags and let AutoPlace pick fresh slots.  Two passes so
	 * stale positions vacated in pass one do not block pass two. */
	extern void AutoPlace();
	FvwmWindow *t;
	int pass;
