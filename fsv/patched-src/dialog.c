...
/* AUX: "Open" context-menu action - regular files open in vim inside
 * an xterm (the box's editor); same xfork/system pattern as the help
 * browser launcher above */
void
dialog_open_node( GNode *node )
{
	char cmdbuf[2048];

	if (xfork( )) {
		/* HOME for /.vimrc, VIMRUNTIME for syntax files (normally
		   set by .bash_profile, absent in fsv's environment);
		   xterm colors = the desktop's synthwave terminals */
		sprintf( cmdbuf,
		    "HOME=/ VIMRUNTIME=/usr/local/src/vim-5.8/runtime "
		    "/usr/local/bin/xterm -geometry 110x45 -fn 7x13 -fg rgb:7a/f0/ff "
		    "-bg rgb:19/0a/2e -cr rgb:ff/2a/8a "
		    "-title \"%s\" -e /usr/local/bin/vim \"%s\" > /dev/null 2>&1",
		    NODE_DESC(node)->name, node_absname( node ) );
		system( cmdbuf );
		_exit( 0 );
	}
}
...
static void
open_cb( GtkWidget *unused, GNode *node )
{
	dialog_open_node( node );
}
...
context_menu( GNode *node, GdkEventButton *ev_button )
{
	...
	if (NODE_DESC(node)->type == NODE_REGFILE)
		gui_menu_item_add( popup_menu_w, _("Open"), open_cb, node );
	if (NODE_IS_DIR(node)) {
