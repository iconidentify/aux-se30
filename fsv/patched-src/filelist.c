...
filelist_icons_init( void )
{
	...
	/* AUX: gdk XPM pixmap creation crashes against XmacII -
	   NULL icons; cells below use plain text */
	for (i = 1; i < NUM_NODE_TYPES; i++) {
		node_type_mini_icons[i].pixmap = NULL;
		node_type_mini_icons[i].mask = NULL;
	}
}
...
filelist_populate( GNode *dnode )
{
	...
	while (node_llink != NULL) {
		...
		gtk_clist_set_text( GTK_CLIST(file_clist_w), row, 0, NODE_DESC(node)->name );
...
filelist_select_cb( GtkWidget *clist_w, GdkEventButton *ev_button )
{
	...
	/* A double-click from button 1: open regular files (one-button
	 * Mac mouse idiom - right-click needs Cmd-click on XmacII),
	 * fly the camera for everything else */
...
filelist_scan_monitor_init( void )
{
	...
	for (i = 1; i <= NUM_NODE_TYPES; i++) {
		...
		if (i < NUM_NODE_TYPES) {
			...
			gtk_clist_set_text( GTK_CLIST(file_clist_w), i - 1, 0, _(node_type_plural_names[i]) );
		}
		else
...
dir_contents_list( GNode *dnode )
{
	...
	for (i = 1; i < NUM_NODE_TYPES; i++) {
		...
		icon = &node_type_mini_icons[i];
		gtk_clist_set_text( GTK_CLIST(clist_w), i - 1, 0, _(node_type_plural_names[i]) );
	}
