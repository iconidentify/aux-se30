...
viewport_cb( GtkWidget *gl_area_w, GdkEvent *event )
{
	...
	/* AUX: XmacII delivers expose/configure during gtk_widget_show,
	   before fsv_load has initialized the scene (fsv_mode is not even
	   FSV_SPLASH yet) - drawing at that point dereferences null scene
	   state. 1999 Linux servers delivered these events later, after
	   setup; the code relied on that ordering. */
