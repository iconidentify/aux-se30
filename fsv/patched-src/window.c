...
window_init( FsvMode fsv_mode )
{
	...
	/* AUX: exit by _exit - GTK/GL teardown (and even libc exit
	   cleanup) crashes against XmacII; the server reclaims all
	   resources on disconnect anyway */
	gtk_signal_connect( GTK_OBJECT(main_window_w), "delete_event", GTK_SIGNAL_FUNC(window_quit_cb), NULL );
...
/* Note: don't forget to remove the "-1" three lines up */
	...
	/* "back" button (AUX: text label - gdk XPM icons crash XmacII) */
	button_w = gui_button_add( hbox_w, "<", on_back_button_clicked, NULL );
	G_LIST_APPEND(sw_widget_list, button_w);
	/* "cd /" button */
	button_w = gui_button_add( hbox_w, "/", on_cd_root_button_clicked, NULL );
	G_LIST_APPEND(sw_widget_list, button_w);
	/* "cd .." button */
	button_w = gui_button_add( hbox_w, "..", on_cd_up_button_clicked, NULL );
	G_LIST_APPEND(sw_widget_list, button_w);
	/* "bird's-eye view" toggle button */
	button_w = gui_toggle_button_add( hbox_w, "Eye", FALSE, on_birdseye_view_togglebutton_toggled, NULL );
	G_LIST_APPEND(sw_widget_list, button_w);
	...
	/* AUX: gdk sets titles via XmbSetWMProperties, which needs Xlib
	   locale machinery our stub setlocale cannot feed - WM_NAME never
	   lands and the WM shows "Untitled". Set it the plain way. */
	gtk_widget_realize( main_window_w );
	XStoreName( GDK_WINDOW_XDISPLAY(main_window_w->window),
	            GDK_WINDOW_XWINDOW(main_window_w->window),
	            "3D File Navigator" );
}
