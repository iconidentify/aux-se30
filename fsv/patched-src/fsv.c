...
main( int argc, char **argv )
{
	...
	globals.fstree = NULL;
	globals.history = NULL;
	/* AUX: an uninitialized fsv_mode reads as 0 == FSV_DISCV. XmacII
	   delivers exposes during gtk_widget_show, before fsv_load runs,
	   and drawing DiscV on a null tree crashes. viewport_cb ignores
	   events while the mode is FSV_NONE. */
	globals.fsv_mode = FSV_NONE;
