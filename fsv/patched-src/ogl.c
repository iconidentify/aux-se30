...
ogl_init( void )
{
	...
	/* AUX: light viewport background like the original SGI fsn (and
	   black maps to a hot-pink cell on our starved 8-bit colormap) */
	glClearColor( 0.72, 0.72, 0.78, 1.0 );
...
{
	/* AUX: don't run GL init on a context that failed to bind */
	if (!gtk_gl_area_make_current( GTK_GL_AREA(gl_area_w) ))
		return;
	ogl_init( );
}
