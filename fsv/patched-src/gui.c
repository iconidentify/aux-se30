...
gui_pixmap_xpm_add( GtkWidget *parent_w, char **xpm_data )
{
	...
	/* AUX: gdk XPM pixmap creation (depth-1 mask via XPutImage) brings
	   down the client against XmacII - same operation class that
	   crashed SimCity mono. Icons are cosmetic; use a label. */
	pixmap_w = gtk_label_new( "::" );
...
{
	/* AUX: gdk XPM pixmap creation crashes against XmacII (see
	   gui_pixmap_xpm_add). A WM icon is cosmetic - skip it. */
}
