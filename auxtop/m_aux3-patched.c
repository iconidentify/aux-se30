    ...
    struct statics *statics;
{
    ...
    /* AUX: upstream never allocated the proc-read buffer (preal stayed
       NULL -> read(2) EFAULT "Bad address") */
    preal = (struct proc *)malloc(v.v_proc * sizeof(struct proc));
    epreal = preal + v.v_proc;

    if (!ptable || !pactive || !preal)
    {
...
update_proc_table()
{
	...
	/* AUX: the u-area CPU-time fields aren't where top expects on
	   3.1.1, so p_pcpu reads 0; use the proc struct's p_cpu (the
	   scheduler's own CPU-usage estimate) as the per-process meter */
	p->p_realcpu = (int)(unsigned char)rp->p_cpu;

	/* AUX: re-read the command every refresh when the u-area read
	   produced a real name - a process whose u-area was swapped out
	   on first sighting (empty/??? name) gets labeled once resident */
	if (u.u_comm[0] != '\0' && u.u_comm[0] != '?')
	{
	    strncpy(p->p_name, u.u_comm, sizeof(u.u_comm));
	    p->p_name[sizeof(p->p_name) - 1] = '\0';
	}
...
/* ===== auxtop raw-process accessor (added for the auxtop TUI) ===== */
/* Returns the next active process's fields in a simple struct, with the
   command name properly NUL-terminated (top's format_next_process left
   it unterminated, scrambling the display). Call exactly p_active times
   after get_process_info. */
#include "auxtop_proc.h"

int auxtop_next(out)
    struct auxtop_proc *out;
{
    struct top_proc *pp;
    ...
    if (pp == NULL)
        return 0;
    out->pid  = (long)pp->p_pid;
    out->uid  = (long)pp->p_uid;
    out->pri  = pp->p_pri;
    out->nice = pp->p_nice;
    out->size = (long)pp->p_size;
    out->stat = pp->p_stat;
    out->secs = (long)(pp->p_time / v.v_hz);
    out->pcpu = (double)pp->p_pcpu;
    out->cpu = pp->p_realcpu;
    strncpy(out->name, pp->p_name, sizeof(out->name) - 1);
    out->name[sizeof(out->name) - 1] = '\0';
    return 1;
}

/* auxtop stable sort: by memory size descending, PID as tiebreaker, so
   the list doesn't reshuffle every frame (CPU-sort jitters on an idle
   box and the R6 xterm's imperfect clear turns that into ghosting). */
int auxtop_compare_size(pp1, pp2)
    struct top_proc **pp1;
    struct top_proc **pp2;
{
    struct top_proc *p1 = *pp1;
    struct top_proc *p2 = *pp2;

    if (p1->p_size != p2->p_size)
        return p2->p_size - p1->p_size;
    return (int)(p1->p_pid - p2->p_pid);
}
